...
public class AndroidGraphics implements Graphics {
    ...
    public void drawImage(Image Image, int x, int y, double rotation) {
        
    	Matrix matrix = new Matrix();
        matrix.setRotate((float)rotation,Image.getWidth()/2,Image.getHeight()/2);
        matrix.postTranslate(x, y);
        canvas.drawBitmap(((AndroidImage) Image).bitmap, matrix, null);
        
    }
    
    public void drawImage(Image Image, int x, int y, double rotation, double scale) {
        
    	Matrix matrix = new Matrix();
        matrix.setRotate((float)rotation,Image.getWidth()/2,Image.getHeight()/2);
        matrix.postScale((float)scale, (float)scale);
        matrix.postTranslate(x, y);
        matrix.postTranslate((int)(-Image.getWidth() * (scale - 1) / 2), (int)(-Image.getHeight() * (scale - 1) / 2));
        canvas.drawBitmap(((AndroidImage) Image).bitmap, matrix, null);
